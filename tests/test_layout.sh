...
# ── каждая пользовательская строка обязана быть двуязычной ───────────────────
# Разбор вынесен в tests/i18n_scan.py: грепом это не сделать (продолжения через
# обратный слэш, многострочные usfc_item_full, тела heredoc), а heredoc с python
# прямо здесь не переживает разбор в bash 3.2, под которым тест иногда гоняют
# на macOS. Причина осознанного исключения пишется комментарием `i18n-ok`
# у самой строки — список внутри теста разъехался бы с кодом.
i18n_hits="$(python3 "$(dirname "$0")/i18n_scan.py" "${SCAN_FILES[@]}" || true)"
if [ -n "$i18n_hits" ]; then
    FAIL=$((FAIL + 1))
    echo "FAIL: пользовательский текст без английской пары:"
    printf '%s\n' "$i18n_hits" | sed 's/^/      /'
fi
CHECKED=$((CHECKED + 1))

...
В меню — номер пункта (`5`, можно несколько сразу: `1 3 5` или `1,3,5`), раздел целиком (`C`/`B`/`S`/`P`), всё сразу (`A`), можно сочетать (`B,S`). На пачку из нескольких пунктов вопрос один, дальше каждый идёт по своим дефолтным ответам без остановок, а в конце показывается сводка: что встало, что упало, что пропущено. `I` (или `?`) — справка по пунктам: что делает каждый, какой у него сейчас статус и как его откатить. `H` — справка по алиасам, `R` — команды отката, `U` — снести сам `usfc`.

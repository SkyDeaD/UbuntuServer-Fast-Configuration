...
При самом первом запуске задаётся один вопрос — «для чего этот сервер?» — и ответ разворачивается в готовый профиль. Отказаться можно сразу: пятый вариант открывает обычное меню.
...
В меню — номер пункта (`5`, можно несколько сразу: `1 3 5` или `1,3,5`), раздел целиком (`C`/`B`/`S`/`P`), всё сразу (`A`), можно сочетать (`B,S`). На пачку из нескольких пунктов все вопросы задаются заранее, одним блоком с пояснениями — что такое zram, нужен ли автозапуск nginx и Docker, нужен ли Cloudflare-плагин с токеном. Дальше пункты идут без остановок, а в конце показывается сводка: что встало, что упало, что пропущено. `V 5` — предпросмотр: показывает, что пункт сделал бы, ничего не меняя. `I` (или `?`, а также `R`) — справка и откат по пунктам: что делает каждый, какой у него сейчас статус, как выключить и как удалить совсем. `H` — справка по алиасам, `U` — снести сам `usfc`.
...
usfc --audit --json       # состояние сервера машиночитаемо
usfc --apply zram --yes   # применить и не переспрашивать
usfc --dry-run            # показать, что было бы сделано
usfc --help               # все флаги списком
```

Полный справочник с готовыми командами — [ниже](#все-флаги).
...
Аргументы можно передать **прямо установщику** — тогда меню не откроется, и терминал для этого не нужен. Это и есть путь для cloud-init и Ansible:

```bash
curl -fsSL https://raw.githubusercontent.com/SkyDeaD/UbuntuServer-Fast-Configuration/main/install.sh \
  | sudo bash -s -- --apply web

# из ветки, с ответами из файла, с предпросмотром — всё то же самое
curl -fsSL .../install.sh | sudo bash -s -- --branch dev --apply zram --yes
curl -fsSL .../install.sh | sudo bash -s -- --apply all --dry-run
```

Установщик забирает себе только `--branch` и `--help`; всё остальное уезжает в `usfc`. Явный `--` отдаёт вниз даже эти два.
...
Справочник, чтобы собрать нужную команду под свою задачу. То же самое короче — `usfc --help`.

### Установка

Установщик забирает себе только `--branch` и `--help`. Всё остальное уезжает установленному `usfc`, и тогда **меню не открывается**, а терминал не нужен вовсе — это и есть путь для cloud-init и Ansible.

```bash
BASE=https://raw.githubusercontent.com/SkyDeaD/UbuntuServer-Fast-Configuration/main/install.sh

curl -fsSL $BASE | sudo bash                              # поставить и открыть меню
curl -fsSL $BASE | sudo bash -s -- --apply web            # поставить и сразу применить профиль
curl -fsSL $BASE | sudo bash -s -- --branch dev           # поставить из ветки dev
curl -fsSL $BASE | sudo bash -s -- --help                 # справка установщика
curl -fsSL $BASE | sudo bash -s -- -- --branch feature/x  # отдать --branch вниз, в usfc
```

| Флаг | Что делает |
|---|---|
| `--branch <ветка>`, `-b` | ставить из указанной ветки, по умолчанию `main` |
| `--help`, `-h` | справка установщика |
| `--` | всё после него уходит в `usfc`, даже если совпадает с флагами установщика |

Без аргументов и без терминала установщик не пытается открыть меню: он говорит, что usfc установлен, и выходит нулём.

### Режим работы — что делать

Ровно один из этих флагов. Без них открывается меню.

| Флаг | Что делает |
|---|---|
| `--apply <что>` | применить пункты и выйти, без меню |
| `--audit` | показать состояние сервера, ничего не меняя |
| `--list` | показать профили и id пунктов |
| `--backups` | показать снимки конфигов |
| `--restore [метка]` | вернуть конфиги из снимка; без метки спросит, какой |
| `--help`, `-h` | справка |
| `--version`, `-V` | версия и выход |

### Модификаторы — как делать

Сочетаются с режимом выше.

| Флаг | Что делает | С чем работает |
|---|---|---|
| `--dry-run` | показать, что было бы сделано, и ничего не менять | `--apply` |
| `--yes`, `-y` | отвечать «да» на вопросы применяемых пунктов | только с `--apply`; для `--restore` — подтверждение отката |
| `--config <файл>` | взять ответы на вопросы из файла | `--apply` |
| `--json` | вывести машиночитаемо | только с `--audit` |
| `--lang ru\|en` | язык на этот запуск, не сохраняется | везде |
| `--no-update` | не проверять обновления самого usfc | везде |
| `--verbose` | сырой вывод команд вместо спиннера | везде |

`--yes` намеренно не глобальный: из 62 вопросов 24 имеют защитный дефолт «нет», и среди них удаление самого usfc, остановка nginx и Docker, выключение unattended-upgrades. В связке с `--apply` эти ветки недостижимы — уже применённые пункты пропускаются, удаление живёт только в меню.

### Что писать в `--apply`

Профиль, id пунктов через запятую или `all`.
...
id пунктов: `newuser` `basepkgs` `cli` `fastfetch` `tmux` `docker` `nginx` `certbot` `dockerlog` `fail2ban` `unattended` `zram` `sshhardening` `ufw`.

`newuser` и `sshhardening` в неинтерактивном режиме пропускаются с явным сообщением: имя с паролем не задефолтишь, а закрыть себе доступ к серверу по чужому конфигу — не та цена за автоматизацию. Уже применённые пункты пропускаются, поэтому повторный запуск — честный no-op.

### Файл ответов для `--config`
...
Читается построчно, без `source`: посторонние ключи не выполняются, а вызывают понятную ошибку.

### Коды возврата

| Код | Когда |
|---|---|
| `0` | применилось, либо применять было нечего |
| `1` | часть пунктов упала |
| `2` | не разобрали, что применять (опечатка в профиле или id), либо `--json` без `--audit` |

### Переменные окружения

| Переменная | Что делает |
|---|---|
| `USFC_APT_LOCK_TIMEOUT` | сколько ждать освобождения блокировки dpkg, по умолчанию 300 с |
| `USFC_BRANCH` | ветка для установщика, то же что `--branch` |
| `USFC_LANG` | язык на этот запуск, то же что `--lang` |
| `NO_COLOR` | отключить цвет; важно наличие, а не значение |

### Готовые команды

```bash
# развернуть веб-сервер на чистой машине одной строкой, без вопросов
curl -fsSL https://raw.githubusercontent.com/SkyDeaD/UbuntuServer-Fast-Configuration/main/install.sh \
  | sudo bash -s -- --apply web --yes

# посмотреть, что будет сделано, и не делать
sudo usfc --apply all --dry-run

# только память: zram, своп и sysctl, без лишних вопросов
sudo usfc --apply zram --yes

# развернуть по своему файлу ответов
curl -fsSL https://raw.githubusercontent.com/SkyDeaD/UbuntuServer-Fast-Configuration/main/install.sh \
  | sudo bash -s -- --config /root/usfc.conf --apply all

# аудит в мониторинг, раз в сутки по cron
sudo usfc --audit --json > /var/lib/node_exporter/usfc.json

# английский интерфейс на один запуск
sudo usfc --lang en --audit

# откатить конфиги к последнему снимку, из скрипта
sudo usfc --backups

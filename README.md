...
usfc --audit --json  # то же машиночитаемо: для мониторинга, Ansible, cron
usfc --list        # профили и id пунктов
usfc --apply web   # применить набор пунктов без меню
usfc --apply zram --yes   # и не переспрашивать по ходу (только с --apply)
usfc --dry-run     # показать, что было бы сделано
usfc --backups     # снимки конфигов
usfc --restore     # вернуть конфиги из снимка
```

`--yes` действует **только вместе с `--apply`** и только на вопросы применяемых пунктов. Глобальным его сделать нельзя: из 62 вопросов 24 имеют защитный дефолт «нет», и среди них удаление самого usfc, остановка nginx и Docker, выключение unattended-upgrades.

...
В меню — номер пункта (`5`, можно несколько сразу: `1 3 5` или `1,3,5`), раздел целиком (`C`/`B`/`S`/`P`), всё сразу (`A`), можно сочетать (`B,S`). На пачку из нескольких пунктов вопрос один, дальше каждый идёт по своим дефолтным ответам без остановок, а в конце показывается сводка: что встало, что упало, что пропущено. `H` — справка по алиасам, `R` — команды отката, `U` — снести сам `usfc`.
...
Он спросит имя и пароль (скрытым вводом, с повтором), добавит в группу `sudo` и предложит **скопировать ключи из `/root/.ssh/authorized_keys`** — без этого новый пользователь просто не сможет зайти, если ты ходишь на сервер по ключу. Пустой пароль допустим: тогда вход только по ключу.
...
**Базовые пакеты** — `micro`, `curl`, `wget`, `git`, `nano`, `unzip`, `htop`, `jq`, `rsync` и ещё несколько вещей, которые обычно ставишь в первую же минуту на любом сервере — в том числе `software-properties-common`, без которого не заработает `add-apt-repository`, нужный дальше для PPA fastfetch. `certbot` отсюда переехал в свой пункт (см. ниже).
